...
void plan_arc(
  ...
  if (ENABLED(ARC_P_CIRCLES) && circles) {
    ...
    for (uint16_t n = circles; n--;) {
      ...
      plan_arc(temp_position, offset, clockwise, 0, extra_data);                      // Plan a single whole circle
    }
...
void G2_G3(const bool clockwise, const ExtraData& extra_data) {
  if (true /*MOTION_CONDITIONS*/) {
    ...
    get_coordinates();   // Get X Y Z E F (and set cutter power)
    ...
    if (arc_offset) {
      ...
      plan_arc(destination, arc_offset, clockwise, circles_to_do, extra_data);
